...
import type {
  FixtureDetailsDto,
  TopPerformerDto,
} from "../components/types/FixtureDetails";

const API_BASE_URL = "https://localhost:62965/api";

export const getCurrentMatches = async () => {
  debugger;
  const response = await axios.get(`${API_BASE_URL}/matches/cricbuzz/current`);
...
export const getLiveFixtures = async () => {
  const response = await axios.get(`${API_BASE_URL}/fixtures/live`);
...
export const searchLiveFixtures = async (searchText: string) => {
  const response = await axios.get(`${API_BASE_URL}/fixtures/search`, {
    params: {
