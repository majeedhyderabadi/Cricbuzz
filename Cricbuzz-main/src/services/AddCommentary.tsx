...
import type { FeedingMatchs } from "./match.types";
import {
  fetchLiveTeams,
  liveFixtures,
  postCommentary,
...
import { showError, showSuccess } from "./common/AlertService";
...
function AddCommentary({
  ...
  useEffect(() => {
    if (selectedMatch && allTeams.length > 0 && liveFixturesList.length > 0) {
      ...
      if (fixture) {
        ...
        console.warn(
          "AddCommentary: No matching fixture found for the selected match",
        );
      ...
      if (foundTeams.length === 2) {
        ...
        if (fixture) {
          const homeTeam = foundTeams.find(
            (t) => t.teamName === fixture.homeTeamName,
          );
          const awayTeam = foundTeams.find(
  ...
  const handleUpdateScore = async (teamName: string) => {
    ...
    const payload: { side: 0 | 1; runsDelta?: number; wicketsDelta?: number } =
      { side };
    ...
    if (delta.runs !== 0) {
      if (isFootball) {
        changeParts.push(
          `${delta.runs > 0 ? "+" : ""}${delta.runs} goal${Math.abs(delta.runs) !== 1 ? "s" : ""}`,
        );
      } else {
        changeParts.push(
          `${delta.runs > 0 ? "+" : ""}${delta.runs} run${Math.abs(delta.runs) !== 1 ? "s" : ""}`,
        );
      }
    }
    if (delta.wkts !== 0) {
      changeParts.push(
        `${delta.wkts > 0 ? "+" : ""}${delta.wkts} wkt${Math.abs(delta.wkts) !== 1 ? "s" : ""}`,
      );
    }
        ...
        <div className="score-cards">
          {teams.map((team) => {
            ...
            const hasPendingChange =
              !!delta && (delta.runs !== 0 || delta.wkts !== 0);
                ...
                <div className="score-detail">
                  ...
                  <div className="score-stepper">
                    <button
                      ...
                      disabled={
                        !isMatchLive || (scores[team.name]?.runs || 0) <= 0
                      }
                  ...
                  <div className="score-detail">
                    ...
                    <div className="score-stepper">
                      <button
                        ...
                        disabled={
                          !isMatchLive || (scores[team.name]?.wkts || 0) <= 0
                        }
