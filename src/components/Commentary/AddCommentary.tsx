...
function AddCommentary({ selectedMatch, onFixtureIdChange, onCommentaryPosted, onScoreUpdated }: AddCommentaryProps) {
  ...
  const getSportType = (): "cricket" | "football" => {
    if (selectedMatch?.sport?.toLowerCase().includes("football") || selectedMatch?.sport?.toLowerCase().includes("soccer")) return "football";
    return "cricket";
  };
  ...
  const parseOvers = (oversStr: string) => {
    ...
    return { overs: parseInt(parts[0] || "0"), balls: parseInt(parts[1] || "0") };
  };
  ...
  const subtractBall = (currentOvers: string): string => {
    ...
    b -= 1;
    if (b < 0) {
      if (o > 0) { o -= 1; b = 5; } else b = 0;
    }
    return formatOvers(o, b);
  };

  const fetchTeams = async () => {
    try { const res = await fetchLiveTeams(); setAllTeams(res); } catch (e) { console.error("AddCommentary: Error fetching teams:", e); }
  };
  const getFixtures = async () => {
    try {
      ...
      if (selectedFixtureId) {
        ...
        if (fixture && matchTeams.length === 2) updateLocalScoresAndOvers(fixture, matchTeams);
      }
    } catch (e) { console.error("AddCommentary: Error fetching live fixtures:", e); }
  };
  ...
  useEffect(() => { fetchTeams(); getFixtures(); }, []);
  useEffect(() => {
    if (selectedMatch && allTeams.length > 0 && liveFixturesList.length > 0) {
      ...
      const fixture = liveFixturesList.find(f =>
        (f.homeTeamName.toLowerCase() === selectedMatch.team1.toLowerCase() && f.awayTeamName.toLowerCase() === selectedMatch.team2.toLowerCase()) ||
        (f.homeTeamName.toLowerCase() === selectedMatch.team2.toLowerCase() && f.awayTeamName.toLowerCase() === selectedMatch.team1.toLowerCase())
      );
      if (fixture) {
        setSelectedFixtureId(fixture.id);
        setMatchStatus(fixture.status);
        onFixtureIdChange?.(fixture.id);
        if (foundTeams.length === 2) updateLocalScoresAndOvers(fixture, foundTeams);
      } else {
  ...
  useEffect(() => {
    if (selectedTeamName) {
      ...
      if (team && team.players.length > 0) setSelectedPlayerId(team.players[0].playerId);
  ...
  let newOversStr = delta === 1 ? addBall(currentOvers) : subtractBall(currentOvers);
  if (totalOversLimit !== null && delta === 1) {
    const currentTotal = parseOvers(currentOvers).overs * 6 + parseOvers(currentOvers).balls;
    const newTotal = parseOvers(newOversStr).overs * 6 + parseOvers(newOversStr).balls;
  ...
  const handleActionSelect = (actionType: string) => {
    if (!isMatchLive) { showError("Error", "Cannot post commentary for a match that is not live"); return; }
  ...
  const BALL_CONSUMING_ACTIONS = new Set(["six", "four", "single", "wicket", "two", "three"]);

  const handlePostCommentary = async () => {
    if (!isMatchLive) { showError("Error", "Cannot post commentary for a match that is not live"); return; }
    if (!selectedActionType) { alert("Please select an action"); return; }
    if (!selectedFixtureId) { alert("No fixture selected. Please select a match first."); return; }
    if (!selectedPlayerId) { alert("Please select a player."); return; }
    if (!selectedTeamName) { alert("Please select a team."); return; }

    const side = getSide(selectedTeamName);
    if (side === null) { alert("Could not determine side for the selected team."); return; }
    ...
    if (actionValue === undefined) { alert(`Unknown action type: ${selectedActionType}`); return; }
    ...
    const extraNoteSuffix = isExtraEligible && selectedExtraRuns > 0
    ...
    try {
      ...
      if (!isFootball) {
        ...
        if (newOvers) {
          setOvers(prev => ({ ...prev, [selectedTeamName]: newOvers as string }));
        }
      } else {
        await updateScoreFixtures(selectedFixtureId, {
          side,
    ...
    <div className="add-commentary-container">
      {selectedMatch && matchTeams.length === 2 ? (
        <div className="match-info-banner">
          <span className="match-info">{selectedMatch.sport}: {matchTeams[0].teamName} vs {matchTeams[1].teamName}</span>
          ...
          <span className="match-status-badge" style={{ background: isMatchLive ? "#10B981" : "#F59E0B", padding: "2px 12px", borderRadius: "12px", fontSize: "11px", fontWeight: "bold", color: "white", marginLeft: "8px" }}>
            {isMatchLive ? "🔴 LIVE" : matchStatus?.toUpperCase() || "SCHEDULED"}
          </span>
          <span className="match-sport-badge" style={{ background: isFootball ? "#10B981" : "#8B5CF6", padding: "2px 12px", borderRadius: "12px", fontSize: "11px", fontWeight: "bold", color: "white", marginLeft: "8px" }}>
      ...
      <div className={`score-control ${!isMatchLive ? "disabled-section" : ""}`}>
        <div className="score-header">
          <h3>SCORE CONTROL</h3>
          <span className="feed-score">{isFootball ? "Goals are updated via commentary actions" : "Runs & wickets updated via commentary actions"}</span>
        </div>
        <p className="score-subtitle">{isFootball ? "Set GOALS for each side. Also logs a commentary entry." : "Set RUNS / WKTS for each side. Also logs a commentary entry."}</p>
                    ...
                    <>
                      ...
                      <div className="score-item overs-item">
                        ...
                        <div className="overs-stepper">
                          <button className="stepper-btn" onClick={() => handleOversChange(team.name, -1)} disabled={!isMatchLive || overs[team.name] === "0.0"}>−</button>
                          <span className="overs-value">{overs[team.name] || "0.0"}{totalOversLimit !== null && <span className="overs-limit"> / {totalOversLimit}</span>}</span>
                          <button className="stepper-btn" onClick={() => handleOversChange(team.name, 1)} disabled={!isMatchLive || isAtMaxOvers(team.name)}>+</button>
                        </div>
                      </div>
                    </>
                  )}
      ...
      <div className={`commentary-section ${!isMatchLive ? "disabled-section" : ""}`}>
        ...
        <p className="commentary-subtitle">
          {isMatchLive ? "Pick a team and player, then tap an action — it pushes straight to the live feed." : `Commentary is disabled while match is ${matchStatus?.toLowerCase() || "scheduled"}`}
        </p>
        ...
        <div className="control-group">
          ...
          <div className="team-selector">
            {teams.map(team => (
              <button key={team.name} className={`team-btn ${selectedTeamName === team.name ? "active" : ""}`} onClick={() => setSelectedTeamName(team.name)} disabled={!isMatchLive}>{team.name}</button>
            ))}
          </div>
        </div>
        ...
        <div className="commentary-controls">
          <div className="control-group">
            ...
            <div className="player-selector">
              <select value={selectedPlayerId} onChange={e => setSelectedPlayerId(e.target.value)} className="player-dropdown" disabled={currentPlayers.length === 0 || !isMatchLive}>
                {currentPlayers.length === 0 ? <option value="">No players available</option> : currentPlayers.map(player => <option key={player.playerId} value={player.playerId}>{player.playerName} ({player.role})</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="control-group">
            ...
            <div className="note-input-group">
              <input type="text" placeholder={isFootball ? "e.g. powerful strike from outside the box" : "e.g. drives it through the covers"} value={note} onChange={e => setNote(e.target.value)} className="note-input" disabled={!isMatchLive} />
            </div>
          </div>
        </div>
        ...
        <div className="quick-actions">
          <p className="quick-actions-title">
            ...
            {selectedActionType && <span className="selected-action-indicator"> • Selected: <strong>{selectedActionType.toUpperCase().replace("_", " ")}</strong></span>}
          </p>
          <div className="action-buttons" style={{ display: "grid", gridTemplateColumns: isFootball ? "repeat(5, 1fr)" : "repeat(4, 1fr)", gap: "8px", opacity: isMatchLive ? 1 : 0.5, pointerEvents: isMatchLive ? "auto" : "none" }}>
            {quickActions.map(action => {
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  title={isWicketDisabled ? `${selectedTeamName} is all out` : undefined}
                  style={{
                ...
                >
                  ...
                  <span className="action-label" style={{ fontSize: isFootball ? "9px" : "10px" }}>{action.label}</span>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "10px" }}>
          <button className={`add-note-btn ${postStatus === "success" ? "success" : ""} ${postStatus === "error" ? "error" : ""}`} onClick={handlePostCommentary} disabled={isPosting || !selectedActionType || !isMatchLive}>
            {!isMatchLive ? "Match Not Live" : isPosting ? "Processing..." : postStatus === "success" ? "✅ Posted!" : postStatus === "error" ? "❌ Failed" : <>Post Commentary <span className="arrow">→</span></>}
          </button>
        </div>
