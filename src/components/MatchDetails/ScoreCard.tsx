...
export type CricbuzzScorecardInnings = {
  ...
  ppData?: Record<string, { ppId: string; ppType: string; ppOversFrom: string; ppOversTo: string; runsScored: number }>;

  wicketsData?: Record<string, { batId: string; batName: string; wktRuns: number; wktNbr: number; wktOver: string }>;

  partnershipsData?: Record<string, { bat1Name: string; bat1Runs: number; bat2Name: string; bat2Runs: number; totalRuns: number; totalBalls: number }>;
...
const BALL_CONSUMING_ACTIONS = new Set(["Single", "Two", "Three", "Four", "Six", "Wicket"]);
...
export function mapFixtureLogToScorecards(fixture: FixtureLog): CricbuzzScorecardInnings[] {
  ...
  const homeInnings: CricbuzzScorecardInnings = {
    ...
    scoreDetails: {
      ...
      runRate: homeOversDecimal > 0 ? (fixture.homeScore / homeOversDecimal).toFixed(2) : "0.00",
  ...
  const awayInnings: CricbuzzScorecardInnings = {
    ...
    scoreDetails: {
      ...
      runRate: awayOversDecimal > 0 ? (fixture.awayScore / awayOversDecimal).toFixed(2) : "0.00",
...
function ScoreCard({ scorecards }: ScoreCardProps) {
  ...
  const batsmen: CricbuzzScorecardBatsman[] = Object.values(innings.batTeamDetails?.batsmenData ?? {});

  const bowlers: CricbuzzScorecardBowler[] = Object.values(innings.bowlTeamDetails?.bowlersData ?? {});
  ...
  return (
    <div className="score-card-list">
      <div className="score-card-tabs">
        ...
        {scorecards.map((item, index) => (
          <button
            ...
            className={selectedInnings === index ? "score-card-tab active" : "score-card-tab"}
      ...
      <section className="score-card" key={`${innings.matchId}-${innings.inningsId}`}>

        {/* INNINGS HEADER */}
        <div className="score-card__header">
          <div>
            <h2>{innings.batTeamDetails?.batTeamName ?? "Team"} Innings</h2>
            <span className="score-card__innings-label">Innings {innings.inningsId}</span>
          </div>

          <div className="score-card__total">
            <strong>

              {innings.scoreDetails?.runs ?? 0}/{innings.scoreDetails?.wickets ?? 0}
            </strong>
                  ...
                  <tr key={batsman.batId}>
                    <td className="score-card__player">
                      <div className="score-card__player-name">
                        ...
                        {batsman.isCaptain && <span className="score-card__badge">C</span>}

                        {batsman.isKeeper && <span className="score-card__badge">WK</span>}
                      </div>
                      <span className="score-card__dismissal">{batsman.outDesc || "not out"}</span>
                    </td>
        ...
        {innings.extrasData && (
          <div className="score-card__summary-row">
            ...
            <span className="score-card__summary-detail">
              ...
              (B {innings.extrasData.byes}, LB {innings.extrasData.legByes}, NB {innings.extrasData.noBalls}, W{" "}

              {innings.extrasData.wides}, P {innings.extrasData.penalty})
            </span>
          </div>
        ...
        <div className="score-card__summary-row score-card__summary-row--total">
          ...
          <span className="score-card__summary-detail">
            ...
            ({innings.scoreDetails?.overs ?? 0} Overs, RR: {innings.scoreDetails?.runRate ?? 0})
          </span>
        </div>
              ...
              <tbody>
                ...
                {bowlers.map((bowler) => (
                  <tr key={bowler.bowlerId}>
                    <td className="score-card__player">
                      <div className="score-card__player-name">{bowler.bowlName}</div>
                    </td>
          ...
          <div className="score-card__details-block">
            ...
            <div className="score-card__chips">
              ...
              {wickets.map((wicket, index) => (
                <span className="score-card__chip" key={`${wicket.batId}-${index}`}>

                  {wicket.wktRuns}-{wicket.wktNbr} ({wicket.batName}, {wicket.wktOver})
                </span>
