    ...
    return (
        <section className="score-card">
            ...
            <div className="score-card__header">
                ...
                <span className={`score-card__badge ${isLive ? "score-card__badge--live" : ""}`}>
            ...
            <div className="score-card__summary-row score-card__summary-row--home">
                ...
                <span className="score-card__summary-detail">
                    {fixture.homeOvers ? `${fixture.homeOvers} overs` : "Yet to bat"}
                </span>
            </div>
            <div className="score-card__summary-row score-card__summary-row--away">
                ...
                <span className="score-card__summary-detail">
                    {fixture.awayOvers ? `${fixture.awayOvers} overs` : "Yet to bat"}
                </span>
            </div>
                    ...
                    <table className="score-card__table">
                        ...
                        <tbody>
                            {topPerformers.length === 0 ? (
                                <tr>
                                    <td colSpan={3} style={{ textAlign: "center", color: "#9fb0ca" }}>
                                        No data yet
                                    </td>
                                </tr>
                            ) : (
                                topPerformers.map((p) => (
                                    <tr key={p.playerId}>
                                        <td className="score-card__player">
                                            <div className="score-card__player-name">{p.playerName}</div>
                                        </td>
                                        <td>{p.teamName}</td>
            ...
            <div className="score-card__section">
                ...
                <div className="score-card-tabs">
                    {(["All", "Home", "Away"] as const).map((side) => (
                        ...
                        >
                            {side === "All" ? "All" : side === "Home" ? fixture.homeTeamName : fixture.awayTeamName}
                        </button>
                    ))}
                </div>
                <div className="score-card__details-block">
                    {filteredCommentary.length === 0 ? (
                        <div className="score-card__detail-item" style={{ justifyContent: "center" }}>
                            <span style={{ color: "#9fb0ca" }}>No commentary yet</span>
                        </div>
                    ) : (
                        filteredCommentary.map((entry) => {
                            const isWicket = entry.action === "Wicket";
                            const isBoundary = entry.action === "Four" || entry.action === "Six";
                            const label = ACTION_LABEL[entry.action] ?? entry.action;
                            const time = new Date(entry.createdAtUtc).toLocaleString(undefined, {
                                month: "short",
                                day: "numeric",
                                hour: "2-digit",
                                minute: "2-digit",
                            ...
                            return (
                                <div className="score-card__detail-item" key={entry.id}>
                                    ...
                                    <div>
                                        <div>
                                            {entry.playerName} <span style={{ color: "#98a6c0", fontSize: "13px" }}>({entry.side})</span>
                                        </div>
                                        {entry.note && <div style={{ fontSize: "12px", color: "#96a4be" }}>{entry.note}</div>}
                                    </div>
                                    <span style={{ color: "#9fb0ca", fontSize: "12px", whiteSpace: "nowrap" }}>{time}</span>
                                </div>
                            );
                        })
                    )}
